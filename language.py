...
unmoddable_letters = ["a", "e", "i", "o", "u", "c", "l"]

def archaic(word):
    #don't mess with capitals if the whole word is upper
    if not word.isupper():
        #decapitalise the initial if it's upper
        if word[0].isupper():
            letter = word[0]
            letter = letter.lower()
    ...
    # One-in-ten chance of a terminal e, subject to suitability
    roll = random.randint(0,99)

    if roll < 20 and not word[-1] in unmoddable_letters:
        word = word + "e"
    return word
